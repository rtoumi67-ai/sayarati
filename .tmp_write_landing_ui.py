...
const roleCards = [
  {
    title: "مالك المركبة",
    english: "For Drivers & Families",
    description:
      "لوحة موحدة لسجل الصيانة، التوصيات الوقائية، حجز الميكانيكي، وتتبع حالة سيارتك من الهاتف.",
    cta: "ابدأ كعميل",
    icon: CarFront,
    bullets: ["سجل رقمي لكل عملية", "تنبيهات زيت وفلاتر", "حجوزات موثوقة وسريعة"],
  },
  {
    title: "الميكانيكي والورشة",
    english: "For Mechanics & Workshops",
    description:
      "واجهة احترافية لاستقبال الطلبات، إدارة المواعيد، مشاركة تقارير واضحة، وبناء ثقة أعلى مع العملاء.",
    cta: "انضم كميكانيكي",
    icon: Wrench,
    bullets: ["طلبات مرتبة حسب الأولوية", "تأكيدات فورية للعملاء", "ملف خدمة يعكس الاحتراف"],
  },
];
  ...
  return (
    <div dir="rtl" className="relative overflow-hidden bg-background text-foreground">
      ...
      <main className="relative z-10">
        ...
        <section id="roles" className="border-t border-border/70 py-20 sm:py-24">
          <div className="container-app">
            <SectionHeader
              eyebrow="Choose Your Role"
              title="مساران واضحان داخل نفس التجربة الراقية"
              description="ابدأ من الدور المناسب لك لتحصل على واجهة وخطوات وتوصيات مصممة خصيصًا لاحتياجك، سواء كنت عميلًا أو ميكانيكيًا."
            />
            ...
            <div className="mt-14 grid gap-6 lg:grid-cols-2">
              {roleCards.map((role) => {
                ...
                return (
                  <GlassCard key={role.title} className="group p-7 sm:p-8">
                    <div className="flex flex-wrap items-start justify-between gap-4">
                      <div>
                        <div className="text-xs font-bold tracking-[0.26em] text-primary/88 uppercase">{role.english}</div>
                        <h3 className="mt-3 text-3xl font-bold text-foreground">{role.title}</h3>
                      </div>
                    ...
                    <p className="mt-5 text-base leading-8 text-muted">{role.description}</p>
                    <div className="mt-6 grid gap-3">
                      {role.bullets.map((item) => (
                        <div key={item} className="flex items-center gap-3 rounded-[22px] border border-border/80 bg-card-2/70 px-4 py-3 text-sm text-foreground/84">
                    ...
                    <div className="mt-7 flex items-center justify-between border-t border-border/70 pt-5">
                      <span className="text-sm text-muted">واجهة مخصصة وخطوة بداية أوضح</span>
                      <Link href="/choose-role" className="btn-secondary h-11 px-5 text-sm">
                        {role.cta}
                        <ArrowLeft className="h-4 w-4" aria-hidden />
                      </Link>
                    </div>
...
Path(r"c:\Users\raouf\OneDrive\Desktop\sayarati-new\app\landing\ui.tsx").write_text(content, encoding="utf-8")
